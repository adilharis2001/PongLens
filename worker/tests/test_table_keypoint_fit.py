...
class ChooseTableTest(unittest.TestCase):
    """Position decides only when support cannot, and only between tables
    that are each independently well supported.

    The failure this rule exists for is match 3794e632: eight tables in
    view, and the per-frame rule chose an EMPTY one 68% of the way to the
    right edge in 16 frames of 16, because an idle table shows all its
    landmarks and a wide lens stretches whatever is near the edge.
    """

    FRAME = (1920, 1080)
    CENTRE = quad_at(760, 500)         # centred in a 1920-wide frame
    EDGE = quad_at(1480, 500)          # hard against the right edge

    def test_one_stable_table_changes_nothing(self):
        """The whole blast radius of this change is 'two stable tables'.
        With one, the answer must be byte-identical to pool_frames."""
        winners = [result(quad_at(700, 500)) for _ in range(6)]
        per_frame = [[w] for w in winners]
        mine, _ = F.choose_table(per_frame, winners, *self.FRAME)
        theirs, _ = F.pool_frames(winners)
        self.assertEqual(mine["quad"], theirs["quad"])
        self.assertEqual(mine["frames_used"], theirs["frames_used"])
        self.assertNotIn("chosen_by", mine)

    def test_a_refusal_is_not_upgraded_into_an_answer(self):
        """A first draft answered whenever ONE stable table existed. On the
        hand-marked corpus that turned a protective refusal into a table 28%
        of the diagonal from the mark. Centrality may add a decision, never
        an answer."""
        winners = []                       # nothing survived the per-frame bar
        edge = result(self.EDGE)
        per_frame = [[dict(edge)] for _ in range(6)]
        pooled, reason = F.choose_table(per_frame, winners, *self.FRAME)
        self.assertIsNone(pooled, reason)

    def test_the_more_central_of_two_stable_tables_wins(self):
        centre, edge = result(self.CENTRE), result(self.EDGE)
        # The per-frame rule picks the edge table every time, as it did in
        # production; both tables are stable, so position decides.
        winners = [dict(edge) for _ in range(6)]
        per_frame = [[dict(edge), dict(centre)] for _ in range(6)]
        pooled, reason = F.choose_table(per_frame, winners, *self.FRAME)
        self.assertIsNotNone(pooled, reason)
        self.assertEqual(pooled["chosen_by"], "centrality")
        self.assertEqual(pooled["quad"], self.CENTRE)

    def test_two_equally_central_tables_refuse(self):
        left = quad_at(360, 500)
        right = quad_at(1160, 500)         # mirror image, same off-centre
        a, b = result(left), result(right)
        per_frame = [[dict(a), dict(b)] for _ in range(6)]
        pooled, reason = F.choose_table(per_frame, [dict(a) for _ in range(6)], *self.FRAME)
        self.assertIsNone(pooled)
        self.assertIn("equally central", reason)

    def test_a_table_seen_in_too_few_frames_is_not_stable(self):
        """Stability is a frame count, not a candidate count: a table that
        turns up in two frames of six cannot outvote one seen in all six."""
        centre, edge = result(self.CENTRE), result(self.EDGE)
        winners = [dict(edge) for _ in range(6)]
        per_frame = ([[dict(edge), dict(centre)] for _ in range(2)]
                     + [[dict(edge)] for _ in range(4)])
        pooled, reason = F.choose_table(per_frame, winners, *self.FRAME)
        self.assertIsNotNone(pooled, reason)
        self.assertEqual(pooled["quad"], self.EDGE)   # unchanged from today

    def test_every_stable_candidate_is_recorded(self):
        """A wrong table must be diagnosable from the stored record rather
        than by re-running the network over the original video."""
        centre, edge = result(self.CENTRE), result(self.EDGE)
        per_frame = [[dict(edge), dict(centre)] for _ in range(6)]
        pooled, _ = F.choose_table(per_frame, [dict(edge) for _ in range(6)], *self.FRAME)
        self.assertEqual(len(pooled["candidates"]), 2)
        self.assertEqual([c["chosen"] for c in pooled["candidates"]], [True, False])
        self.assertEqual([c["frames_seen"] for c in pooled["candidates"]], [6, 6])
        self.assertLess(pooled["candidates"][0]["off_centre"],
                        pooled["candidates"][1]["off_centre"])

    def test_off_centre_is_a_fraction_of_the_half_width(self):
        self.assertAlmostEqual(F.off_centre(quad_at(760, 500), 1920), 0.0, places=2)
        self.assertGreater(F.off_centre(self.EDGE, 1920), 0.6)
