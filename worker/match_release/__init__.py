...
MINIMUM_DATABASE_MIGRATION = '20260916120000'
...
def build(repo, output, config, commit='HEAD'):
    ...
    with tempfile.TemporaryDirectory(prefix='.building-', dir=output) as temporary:
        ...
        manifest = {'schema': 1, 'source_commit': commit,
                    'database': {
                        'minimum_migration': MINIMUM_DATABASE_MIGRATION,
...
def prepare_run(path, state, lane='main'):
    ...
    env.update({
        ...
        'PONGLENS_MINIMUM_DB_MIGRATION': manifest['database']['minimum_migration'],
        'PONGLENS_CANONICAL_PUBLICATION_CONTRACT': str(
            manifest['database']['canonical_publication_contract']),
