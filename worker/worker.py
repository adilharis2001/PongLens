    ...
    def wrapped(conn, msg):
        ...
        try:
            ...
            _feedback_telemetry = ProcessingTelemetry(
                ...
                processing_outcome.release_identity()[0], _queue_feedback_event)
        except Exception:
            _feedback_telemetry = None
        try:
...
def update_job(conn, job_id: str, **fields):
    ...
    if _feedback_telemetry and _feedback_telemetry.job_id == str(job_id):
        if "progress" in fields:
            _feedback_telemetry.emit("progress", progress=fields["progress"],
                                     stage=_pulse_state.get("stage"))
...
def send_failure_emails(conn, e: Exception, job_id: str | None, kind: str,
    ...
    # event. A deterministic failure is terminal on its first attempt, so
    # the private-video and wrong-sport messages still go out at once.
    if not terminal:
        return
...
def players_device(conn) -> str:
    """Which chip reads the players: app_config.players_device.

    Same contract as points_pipeline and game_end_detection — read per job so
    one UPDATE moves the body stage between the processor and the GPU with no
    deploy and no restart, and a config read that errors FAILS SAFE to the
    processor, which is slower and can never be wrong for a new reason.
    PONGLENS_PLAYERS_DEVICE still overrides, for a one-off run by hand.
    """
    env = os.environ.get("PONGLENS_PLAYERS_DEVICE")
    if env:
        return env
    try:
...
def process_job(conn, msg) -> None:
    ...
    try:
        ...
        if not options.get("points"):
            _record_video_profile(local_input, "legacy_spans", camera_offset_s)
        ...
        if options.get("points"):
            ...
            try:
                ...
                if points_kwargs.get("pipeline") == "bodies":
                    # THE BODY-FIRST ASSEMBLER (spec 2026-09-08). Read the
                    # players, then rebuild the points with the bodies
                    # deciding. Fails open to the cards already on disk.
                    outdir = run_body_points_pass(
                        conn, job_id, local_input, blurball_out, workdir,
                        options, points_kwargs=points_kwargs)
...
# so the two never write over each other's pulse row (2026-09-16).
...
_pulse_state: dict = {
    ...
    # The queue message this job came from. With two machines reading
    # one queue, a message must never reappear while its job is still
    # being worked on; the pulse pushes it out again every beat.
    "msg_id": None,
}
...
def _pulse_once(conn) -> None:
    ...
    with conn.cursor() as cur:
        ...
        # The claim itself. pgmq hides a message for VISIBILITY_S when
        # it is read and nothing extended that until the cloud twin
        # existed: a job over thirty minutes reappeared to any OTHER
        # reader while this one was still on it. Pushing the window out
        # on every beat keeps a held job invisible for as long as the
        # process holding it is alive, and no longer. Best-effort like
        # the rest of the pulse: a missed extension costs nothing that
        # the next beat does not repair.
...
def main():
    ...
    # ...and on exactly one MACHINE. The cloud twin runs a main lane too
    # and starts it with PONGLENS_HOUSEKEEPING=0 (2026-09-16).
