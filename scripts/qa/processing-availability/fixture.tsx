...
function Preview() {
  ...
  return <>
    <aside className="border-b border-edge p-3 text-sm" aria-label="Preview controls">
      <label>Screen <select value={screen} onChange={(e) => setScreen(e.target.value)}>{["match", "home", "library", "admin", "upload", "clip", "uploading", "exports"].map((s) => <option key={s}>{s}</option>)}</select></label>{" "}
