...
For early-outcome checks, replay unanswered point 2 and tap either winner or Skip while more than 3.5 seconds remain. Each action must keep the remaining full card playing and ARM the second answer, without adding a pause: both winner buttons take a "2nd" marker, neither shows the card's existing answer, and the hint names the card. Tapping a winner OR Skip then cuts the card at that mark, gives the new half that answer, and arms again if a third rally's worth of footage is left; neither stops the footage. Undo rejoins the halves. Verify the hint's Split pill pauses to open Modify with the suggested cut, and that tail completion, navigation, Undo or a correction disarms it. Late answers and corrections must not arm anything. Use the delayed/failing transport cases to check that failed saves retire only their own current offer and cannot advance an unanswered point.
...
Only `PATCH http://127.0.0.1:54321/rest/v1/points?id=eq.<fixture-point-id>` is eligible for a 204 response, plus the two cut calls — `POST /rest/v1/rpc/split_point` and `POST /rest/v1/rpc/unsplit_point` — which answer 200 with the new row and with null. A split mints the half it returns, and only a minted id joins the point allowlist; a request can never introduce one. The body must contain exactly `confirmed_winner`, `is_let`, and `scored_at_cut_s`, with valid coupled values; every other target, method, query shape, field, type, or oversized body receives 403 and is never forwarded. Unexpected-event diagnostics contain only method, path, query-key names, and whether Foundation supplied body data or a body stream.
