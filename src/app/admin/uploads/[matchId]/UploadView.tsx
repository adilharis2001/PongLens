    ...
    <div className="mx-auto w-full max-w-3xl px-4 pt-6 sm:px-6 lg:max-w-6xl">
      ...
      <section className="mt-8">
        ...
        {serveMisses && serveMisses.cards.length > 0 && (
          <div className="mt-3 rounded-2xl border border-edge bg-surface p-4">
            <p className="text-sm text-zinc-300">
              {serveMisses.cards.length} of {serveMisses.total_cards} cards
              were built without a serve. A serve is accepted as a PAIR of
              bounces, so a card with none is a card where no pair passed the
              six rules. Open any card below to watch what the detector was
              looking at.
            </p>
            {/* Two counts for one thing, a screen apart, reads as a bug —
                and here it is real: the stored diagnosis was generated by an
                earlier run, and two of the serve rule's constants are read
                per job from app_config, so a settings change moves the
                answer. Say which number is which rather than letting them
                quietly disagree. */}
            {assembly.cardsWithoutServe !== null &&
              assembly.cardsWithoutServe !== serveMisses.cards.length && (
                <p className="mt-2 text-sm text-amber-300">
                  This match now has {assembly.cardsWithoutServe} cards with no
                  serve, so the diagnosis below predates the settings it runs
                  under and explains {serveMisses.cards.length} of them.
                </p>
              )}
            <div className="mt-3 flex flex-wrap gap-1.5">
              {reasonTally(serveMisses.cards).map(({ reason, count }) => (
                <span
                  key={reason}
