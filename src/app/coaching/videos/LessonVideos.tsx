...
export function LessonVideos({students,userId,initialStudent}:{students:{id:string;display_name:string}[];userId:string;initialStudent:string}){
 ...
 async function load(){try{const r=await fetch('/api/lesson-video'+(initialStudent?'?studentId='+encodeURIComponent(initialStudent):''));const d=await r.json();if(!r.ok)throw new Error(d.error);if(active.current)setVideos(d.videos);}catch(e){if(active.current)setError(String((e as Error).message));}}
 useEffect(()=>{active.current=true;void load();const timer=setInterval(()=>{void load();},10000);return()=>{active.current=false;clearInterval(timer);};},[initialStudent]);
 ...
 return <main className="text-zinc-100"><Link href={initialStudent?`/coaching/students/${initialStudent}`:"/coaching"} className={button}>{initialStudent?"Back to student":"Back to Coaching"}</Link><h1 className="mb-8 mt-8 text-2xl font-bold tracking-tight sm:text-3xl">Lesson videos</h1><div className="rounded-2xl border border-edge bg-surface p-5"><h2 className="text-base font-semibold">Import a lesson</h2><p className="mt-4 max-w-2xl text-sm leading-relaxed text-zinc-400">Record in the iPhone Camera app at 1080p and 30 fps, in landscape. Place the tripod near the coach, angled across the table. Check that you can hear their explanation over the ball sounds.</p><p className="mt-3 text-sm text-zinc-400">Lessons up to three hours and 20 GB. Your original and recap are kept until you delete them.</p><label className="mt-6 block text-sm">Student<select disabled={busy||!!initialStudent} className="mt-2 block w-full max-w-md rounded-xl border border-zinc-700 bg-zinc-950 p-3" value={student} onChange={e=>setStudent(e.target.value)}><option value="">Private lesson</option>{students.map(s=><option key={s.id} value={s.id}>{s.display_name}</option>)}</select></label><input ref={input} type="file" accept="video/mp4,video/quicktime,video/*" className="hidden" onChange={e=>{const f=e.target.files?.[0];if(f)void upload(f);}}/><button disabled={busy} className={button+' mt-5 bg-cyan-950'} onClick={()=>input.current?.click()}>{busy?'Uploading…':'Choose a video'}</button>{busy&&<p className="mt-3 text-sm text-zinc-400">Keep this page open during upload. For a long recording, the iPhone app also saves upload progress.</p>}{progress&&<p role="status" className="mt-4 text-cyan-300">{progress}</p>}{error&&<p role="alert" className="mt-4 text-amber-300">{error}</p>}</div><div className="mt-8 space-y-3">{videos.map(v=><Link className="flex items-center justify-between gap-4 rounded-2xl border border-edge bg-surface p-5 text-sm hover:bg-surface-2" key={v.id} href={'/lesson-video/'+v.id}><div><p className="font-medium">{v.edit?.title??v.original_name}</p><p className="mt-1 text-sm text-zinc-400">{new Date(v.created_at).toLocaleDateString()} · {Math.round(v.duration_s/60)} minutes</p></div><span className="text-sm text-cyan-300">{lessonStatusLabel(v,!!v.shared)}</span></Link>)}</div></main>;
}
