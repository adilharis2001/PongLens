...
export function LessonVideos({students,coaches=[],audience='coach',userId,initialStudent,uploadConfirmed}:{students:{id:string;display_name:string}[];coaches?:{id:string;display_name:string}[];audience?:'coach'|'player';userId:string;initialStudent:string;/** player_profiles.upload_confirmed_at is set. False shows the one-time checkbox and keeps the button off until it is ticked. */uploadConfirmed:boolean}){
 ...
 // Written when the upload starts, not when the box is ticked. Ticking used to write it at once, so somebody who ticked and then left without uploading was confirmed for good and their first real upload had nothing in front of it. The box says "this video", so the answer belongs to the upload — and until then the tick can be taken back. False only when the write failed; the route checks the same column and would refuse anyway.
 const saved=useRef(uploadConfirmed);
 async function saveConfirmation(){
  ...
  setConfirming(false);if(writeError){setConfirmError("We couldn't save that. Try again.");return false;}
  saved.current=true;return true;
 }
 ...
 async function upload(file:File){setBusy(true);setError('');setProgress('Reading the video…');let objectUrl='';
  try{
   // The upload starts here, so this is where the tick is saved. A box that was ticked and then abandoned confirms nothing.
   if(!(await saveConfirmation())){setProgress('');return;}
