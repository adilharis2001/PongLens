...
export default async function Page() {
  ...
  const {data,error} = await db.from('active_ball_samples').select('id,match_id,venue,split,frame,time_s,width,height,frame_keys,corners,label,prediction,model_run,revision').order('created_at').order('id').limit(500);
