...
export function SuggestionHint({state,detail,onConfirm}:{state:SuggestionState;detail?:string;onConfirm?:()=>void}) {
 ...
 return <div className={`mt-2 text-xs ${state==='pending'?'text-amber-200':state==='uncertain'?'text-zinc-500':'text-cyan-100'}`}>
  ...
  {state==='pending'&&detail&&<p className="mt-1 text-zinc-400">{detail}</p>}
 </div>;
}
