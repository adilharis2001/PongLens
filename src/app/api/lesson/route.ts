...
- "${GOALS_HEADING}", up to ${MAX_GOALS} points: what the session set out to improve. Only where the coach or the player actually said what they were working on or why they were doing it, as in "I'm going to address timing and tempo" or "let's work a little bit on the feet". A drill happening is not a goal. Do not turn an ordinary correction into one: "stand a little bit wider" is an instruction, and writing it as "you are working on a wider stance" states a purpose nobody gave. Most sessions state one or two, and fewer is right far more often than reaching for ${MAX_GOALS}. These points are the one exception to the sentence rule above: write each as a short phrase beginning with a verb ending in -ing, roughly five to ten words, naming what was being built, as in "Building body rotation into the forehand loop". No second person, no full stop, and never an instruction, because the other reserved heading is the list of instructions and the two must read differently. Leave the heading out entirely when the session never says.
