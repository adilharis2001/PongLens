...
export type StartReviewCase={pointId:string;runId:string;proposedStart:number;recordedServeTap:number|null};
export function validStartReview(value:unknown):value is StartReview {
 if(!value||typeof value!=='object'||Array.isArray(value))return false;
 const x=value as StartReview;
 return x.runId===START_REVIEW_RUN_ID&&[null,'yes','no','uncertain'].includes(x.outcome)&&
...
export function startReviewAllowed(review:StartReview,existing:StartReview|undefined,serveStart:CutBoundary,item?:StartReviewCase) {
 return !!item&&validStartReview(review)&&review.runId===item.runId&&
  (review.observedServeStart===serveStart||!!existing&&review.runId===existing.runId&&review.outcome===existing.outcome&&review.observedServeStart===existing.observedServeStart);
}
