...
export const TEST_AREAS = [
  ...
  { key: "placement", title: "Detailed analysis" },
...
export const testCases: TestCase[] = [
  ...
  },
  {
    ...
    why: "Onboarding asks for a name, then whether this account is here to play or to coach, then how they heard about us, then the playing profile: handedness, grip and rubbers, because a coach reading a match needs them. A coach answers everything but the playing profile and lands on the coaching side. It is offered once and can be skipped, and being asked twice is a bug.",
    needs: ["An account that has never signed in"],
    steps: [
      "Sign in with a brand new address.",
      "Complete or skip each onboarding step.",
      "On the source question, pick A coach and type a name; pick YouTube and check the name field goes away.",
      "Sign out, sign back in.",
    ],
    expected: [
      "Onboarding appears on the first sign-in only.",
      "Skipping is allowed and does not block the app.",
      "Only the four answers that name somebody open a name field, and switching answers clears what was typed.",
      "The answer and the name appear against that account in the From column on /admin/players.",
  ...
  // -------------------------------------------------------------------------
  {
    ...
    area: "placement",
    title: "The detailed analysis is generated from Match analysis after scoring",
    why: "The analysis reads the confirmed points, so it is offered only once the match is scored. A match that never had it shows no maps, and that is correct rather than a bug. Since August 2026 the maps show serves only: fewer dots that are almost always right, rather than every bounce.",
    needs: ["A scored match that has not had the detailed analysis"],
    steps: ["Open Match analysis, swipe to the last card, and press Generate detailed analysis."],
    expected: [
      "The card shows Generating, then the serve maps and the video cards appear.",
      "A match without the analysis shows no maps and does not pretend to.",
    ],
    surfaces: ALL,
    depth: "core",
    added: "2026-09-08",
  ...
  {
    id: "sharing-coach-roster",
    area: "sharing",
    title: "Every coach reaches their own page, whatever their standing",
    why: "Your coaches used to be built on invite links, so a coach the player had only written down while filing a lesson appeared nowhere on the page that is meant to list coaches. The permanent row on the Coaching tab was added at the same time, because the only link to that page used to live inside the first-run card and writing down one coach hid it.",
    needs: ["A player account with a written-down coach, a connected coach and an invite nobody named"],
    steps: [
      "Open Coaching and read the row under the chips.",
      "Open Your coaches.",
      "Open the written-down coach.",
    ],
    expected: [
      "The Coaching tab shows a permanent Your coaches row, with the count of invites still waiting when there is one.",
      "The roster lists all three: the written-down coach, the connected one, and the unnamed invite with Copy link and Revoke.",
      "The written-down coach's page offers Send an invite and Remove from your list, not only Rename.",
      "It reads the same on the phone, on desktop web and at 393x660.",
    ],
    surfaces: ALL,
    depth: "core",
    added: "2026-09-10",
  ...
  {
    id: "sharing-coach-remove-restore",
    area: "sharing",
    title: "Removing a coach is reversible and keeps the lessons' names",
    why: "Removal archives rather than deletes, because both foreign keys onto the row are on delete set null and that null blanks coach_name on every entry the coach ever taught. Ending access and removing are separate actions on purpose: one stops them watching and keeps them on the list, the other takes them off it.",
    needs: ["A player account with a coach who has at least one attributed lesson"],
    steps: [
      "Open the coach and press Remove from your list, then confirm.",
      "Open the Journal and find a lesson that named them.",
      "Go back to Your coaches, open Removed coaches and press Put back.",
      "Open the removed coach's page by its old address, before putting them back.",
    ],
    expected: [
      "The confirmation says what happens, and that your lessons keep their name.",
      "You land on Your coaches, and the coach is under Removed coaches.",
      "The lesson still says who taught it.",
      "Put back returns them to the list.",
      "Their page does not 404 while removed: it says you removed them, and offers Put back.",
    ],
    surfaces: ALL,
    depth: "core",
    added: "2026-09-10",
  },
  {
    ...
    steps: [
      "Create a coach invite from Coaching, then Your coaches, then Add a coach.",
