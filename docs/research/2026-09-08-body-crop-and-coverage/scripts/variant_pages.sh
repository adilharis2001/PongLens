...
TAG=${TAG:-variant}; LOG=$F/variant_$TAG.log
...
trap restore EXIT
EXTRA="${EXTRA:-}"; SW=""
log "variant $TAG: extra sets: $EXTRA (poses unchanged)"
log "dump (deploy command + $EXTRA)"
V3_BF_SMOOTH=0.5 nice -n 12 "$PY" bodyfirst.py VFINAL2 $MS --holdout $HOLD --fam rhythm,floor,snap,ball --set ball_floor=0.6 --set ball_floor_tempo=3 --set dur_w=8.0 --set snap_on=1.0 $EXTRA --dump > $F/bf_${TAG}_dump.log 2>&1; log "  exit $?"
