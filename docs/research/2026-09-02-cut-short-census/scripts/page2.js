...
(function () {
  'use strict';
  var KEY = 'ponglens-cutshort-v1';
  var SEEN = 'ponglens-cutshort-seed';
  var marks = {};
  ...
  function apply(value, quiet) {
    ...
    cards.forEach(function (c) {
      var show = want === 'all'
        || (want === 'real' ? c.dataset.verdictSeed === 'real'
           : want === 'no' || want === 'unsure' ? c.dataset.verdictSeed === want
           : c.dataset.cause === want);
  ...
  function report() {
    var lines = ['match\tpoint\tcause\tverdict'];
    cards.forEach(function (c) {
      lines.push([c.dataset.match, c.dataset.t, c.dataset.causeName || '',
