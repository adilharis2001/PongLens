...
The Mac's own cut does not work that way: it encodes each segment
separately and joins the parts, each a few milliseconds longer than its
window, so its clock runs ahead of the plan (0.6 s by the end of an
86-segment match). Since 2026-09-25 it publishes the offsets `cmd_cut`
measured, never the plan's (`docs/research/2026-09-25-hand-cut-clock/`).
