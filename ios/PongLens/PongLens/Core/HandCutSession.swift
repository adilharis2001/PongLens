...
/// The seam for playing a file that is already on the phone.
///
/// Today every session streams the original upload through a presigned link
/// minted once and frozen for the session, as the web does. A later phase
/// keeps the recording on the phone (`LocalMatchVideos.url(for:)`), and the
/// marker should prefer it: set `localFile` to that lookup and nothing else
/// changes, because the marker only ever sees a URL.
enum HandCutVideo {
    static var localFile: @MainActor (UUID) -> URL? = { _ in nil }
}
