-- Accept a single human label when paddle contact and table contact are too
-- close in time to separate. This only replaces the assignment validator;
-- existing labels and every lifecycle/read-only guard remain unchanged.
...
begin
  ...
  loop
    ...
    if event_kind is not null and event_kind not in (
      'paddle', 'table', 'paddle_table', 'floor', 'shoe', 'shoe_squeak',
      'stomp', 'net', 'background', 'other', 'no_impact', 'unsure'
    ) then
