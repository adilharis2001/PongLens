-- 059: Move the re-enable backfill onto recollect-v3.
--
-- v3 changes what a candidate has to clear before it is stored: the quality
-- gate now reads the source words behind each cue, and a question that gives
-- away its own answer is rejected outright. Jobs are unique per
-- (lesson, content_hash, processor_version), so a user who turns Recollect
-- back on should be re-processed under the current gate rather than matching
-- a v2 row and being skipped.
